...
export async function registerAuthor(credentials: RegisterRequest): Promise<AuthSession> {
  const { data } = await authHttp.post<AuthSession>('/auth/register', credentials);
  useAppStore.getState().setAuth(data.accessToken, data.user);
  return data;
}
